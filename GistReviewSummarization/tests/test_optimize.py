from summarization.optimize import *
...
@pytest.fixture()
def setup_sentences():
    overall_sentiment, sentence_dataset = summarize.get_attributes(text, name)
    return sentence_dataset, overall_sentiment

def test_genalg_keypoints(setup_sentences):
    culled_sentences, overall_sentiment = setup_sentences

    key_points = genalg_keypoints(culled_sentences, overall_sentiment)
    assert len(key_points) == NUM_KEY_POINTS

def test_pso_keypoints(setup_sentences):
    culled_sentences, overall_sentiment = setup_sentences

    key_points = pso_keypoints(culled_sentences, overall_sentiment)
    assert len(key_points) == NUM_KEY_POINTS

def test_gsa_keypoints(setup_sentences):
    culled_sentences, overall_sentiment = setup_sentences

    key_points = gsa_keypoints(culled_sentences, overall_sentiment)
    assert len(key_points) == NUM_KEY_POINTS